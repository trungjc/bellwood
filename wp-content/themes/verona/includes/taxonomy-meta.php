...
class RW_Taxonomy_Meta {
		...
		foreach ( $this->_fields as $field ) {
			...
			$('#rw_upload_$id').click(function(){
				...
				window.send_to_editor = function(html) {
					...
					html += '<a title=\"" . __( 'Delete this image', 'Verona' ) . "\" class=\"rw-delete-file\" href=\"#\" rel=\"$rel!' + img_id + '!$nonce_delete\">" . __( 'Delete', 'Verona' ) . "</a>';
	...
	function show_field_file( $field, $meta ) {
		...
		if ( !empty( $meta ) ) {
			...
			foreach ( $meta as $att ) {
				...
				echo "<li>" . wp_get_attachment_link( $att ) . " (<a class='rw-delete-file' href='#' rel='$rel!$att!$nonce'>" . __( 'Delete', 'Verona' ) . "</a>)</li>";
			}
			echo '</ol>';
		}

		// show form upload
		echo "<div style='clear: both'><strong>" . __( 'Upload new files', 'Verona' ) . "</strong></div>
			<div class='new-files'>
				<div class='file-input'><input type='file' name='{$field['id']}[]' /></div>
				<a class='rw-add-file' href='javascript:void(0)'>" . __( 'Add more file', 'Verona' ) . "</a>
			</div>
		</td>";
	}
	...
	function show_field_image( $field, $meta ) {
		...
		foreach ( $meta as $att ) {
			...
			echo "<li id='item_{$att}'>
					<img src='$src' />
					<a title='" . __( 'Delete this image', 'Verona' ) . "' class='rw-delete-file' href='#' rel='$rel!$att!$nonce_delete'>" . __( 'Delete', 'Verona' ) . "</a>
					<input type='hidden' name='{$field['id']}[]' value='$att' />
				</li>";
		}
		echo '</ul>';

		echo "<a href='#' style='float: left; clear: both; margin-top: 10px' id='rw_upload_{$field['id']}' class='rw_upload button'>" . __( 'Upload new image', 'Verona' ) . "</a>";
	}
	...
	function show_field_color( $field, $meta ) {
		...
		echo "<input type='text' name='{$field['id']}' id='{$field['id']}' value='$meta' size='8' style='{$field['style']}' />
			  <a href='#' id='select-{$field['id']}'>" . __( 'Select a color', 'Verona' ) . "</a>
