...
class SYMPLE_TinyMCE_Buttons {
    ...
    function enqueue_admin_style() {
       if ( version_compare( get_bloginfo('version'), '3.9', '>=' ) ) {
            wp_enqueue_style('sympleshortcodes-admin', plugin_dir_url( __FILE__ ) . 'css/symple_shortcodes_tinymce.css', '1.0', 'all');
        }
    }
	function add_plugin($plugin_array) {
       if ( version_compare( get_bloginfo('version'), '3.9', '>=' ) ) {
          ...
          $plugin_array['symple_shortcodes'] = plugin_dir_url( __FILE__ ) .'js/symple_shortcodes_tinymce_pre3.9.js';
       }
	   return $plugin_array; 
	}
